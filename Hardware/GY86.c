...
void MyI2C_SendByte(uint8_t Byte)
{
    ...
    for(i=0;i<8;i++)
		{
				SDA_W(Byte & (0x80>>i));//高位先行（MSB first）
				SCL_W(1);//拉高时钟：数据被读取（从机读取 SDA 的值）
				SCL_W(0);//拉低时钟：数据被传输（主机在 SDA 线上发送数据位）
		}
}
...
void GY86_GPIO_Config(void)
{
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE);
	
	GPIO_InitTypeDef GPIOInitStructure;
	GPIOInitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIOInitStructure.GPIO_OType = GPIO_OType_OD;//开漏输出
...
void MPU6050_Init(void)
{
	...
	GY86_WriteReg(MPU6050_ADDRESS, MPU6050_GYRO_CONFIG, 0x18);		//配置陀螺仪量程
	GY86_WriteReg(MPU6050_ADDRESS, MPU6050_ACCEL_CONFIG, 0x18);		//配置加速度计量程
}
